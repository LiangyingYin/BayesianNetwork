######################################################################################
# Please make sure that you install the corresponding R package before running the simulation code.


######################################################################################
# Load the source code and function code
# /opt/R-3.6.1/bin/R
FOLDER_PATH = "/exeh_4/yaning_feng/04_Simulation/Github_code/" # Please amend this path to your local path

source(paste0(FOLDER_PATH,"Simulation_Extension_Functions/Simulation_ExtensionFunction_Fisher_IDA_topK.R"))
source(paste0(FOLDER_PATH,"Simulation_Extension_Functions/Simulation_ExtensionFunction_Bootstrap.R"))
source(paste0(FOLDER_PATH,"Simulation_Extension_Functions/Simulation_ExtensionFunction_Mediator_functions.R"))
source(paste0(FOLDER_PATH,"Simulation_Extension_Functions/Simulation_ExtensionFunction_pcSelect_stableVer.R"))


source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/01_Simulation_GenerateSamples.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/02_Simulation_GeneToGeneNetwork_Inference.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/03_Simulation_GeneToPhenoNetwork_Inference.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/04_Simulation_Merge.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/05_Simulation_UnivariateTest.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/06_Simulation_StrategyForAdjustment.R"))
source(paste0(FOLDER_PATH,"Simulation_Steps_Functions/07_Simulation_OrganizeSummarizeResults.R"))
...
result_path = paste0(csv_path,"Res/")
...
# set.seed(666) # This is for testing only, please delete this when run simulation to ensure different duplicates randomly
...
for (x in 1:rows_testdata)
{
  ...
  #Parameter setting
  no_sim = 0 
  total_sim = 2 # Set the total number of simulation you need
  try_res = 0
  ...
  repeat{
    ...
    try_res = try({
      ...
      #############################################################################
      # setting the parameter in rmvDAG function in R
      ...
      set.seed(666) # This is for testing only, please delete this when run simulation to ensure different duplicates randomly
      # samples_data2 <- rmvDAG(sample_num2, rDAG, errMat = eMat) # Can add different kind of noise
      samples_data2 <- rmvDAG(sample_num2, rDAG, errDist="normal") # If meet topologically order issue, re-run to generate another graph instead
