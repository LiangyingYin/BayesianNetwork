...
# /opt/R-3.6.1/bin/R
FOLDER_PATH = "/exeh_4/yaning_feng/04_Simulation/Github_code/BackupV1.1/" # Please amend this path to your local path
...
csv_path =  "/exeh_4/yaning_feng/04_Simulation/Github_code/BackupV1.1/"
...
for (x in 1:rows_testdata)
{
  ...
  repeat{
    ...
    try_res = try({
      ...
      # seed1 = sample(10000,1,replace = FALSE)
      set.seed(5662)
      ...
      set.seed(5662) # Set seed is only for testing, should be deleted if you want to generate different replicates
      samples_data2 <- rmvDAG(sample_num2, rDAG, errMat = eMat)
      # fwrite(seed1,file = paste0(result_path,"seed_info.csv"))
  ...
  if(nrow(epi_test_result_mediator_detection_ByGraphStructure)>1)
  {
