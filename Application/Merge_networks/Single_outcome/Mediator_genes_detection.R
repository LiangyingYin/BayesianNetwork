...
# VersionV1.0 added by fyn 29/3/2021
# Input: The adjacency matrix of a graph.1:nrow(adj) should be the gene, final col and row is outcome
# Output:a list, which show if the gene has mediator or not, 0 stand for no mediators, otherwise the num is the distance to outcome.
# More Detail: Output include.
...
Check_Mediator <- function(adj)
{
  ...
  for(i in 1:length(differ_list))
  {
    ...
    if(ifmediator==0)
    {
      ...
      #set the direct relation to 0, then get the distance.
      # graph.fit_tmp = as(adj_tmp, "graphNEL")
...
Check_Mediator2 <- function(adj)
{
  ...
  # Revised by yinly
  # Description: the last row is the outcome, so we to remove it when perform mediator analysis
  ifmediator_gene = seq(1,(nrow(adj)-1))
  gene_type = seq(1,(nrow(adj)-1))
  ifmediator_gene[]=0
  gene_type[]=0
  #for(i in 1:nrow(adj_gene_outcome))
  for(i in 1:(nrow(adj_gene_outcome)-1))
  {
    adj_tmp = adj_gene_outcome
