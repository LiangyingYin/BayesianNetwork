...
Get_Causal_effects_of_outcome_related_genes <- function(bgadd,pc_graph_name,Selected_genes_name,target_outcome_name,Merged_graph_name,predicted_trait_name){
  #*************************************************************************************************
  ...
  cat("There are",nrow(selectedMatrix),"nodes remaing after distance filteration for",target_outcome_name,"\n")
  ...
  # Add pMax row and column for genetically predicted clinical trait
  rho_pMax_predicted_trait = 2*pnorm(-abs(Selected_genes_all$zMin.x))
  rho_pMax_predicted_trait[which(is.na(rho_pMax_predicted_trait))] = -1
  len_predictors = length(rho_pMax_predicted_trait)
  ...
  rho_pMax_predictors[len_predictors,] = rho_pMax_predicted_trait
  rho_pMax_predictors[,len_predictors] = rho_pMax_predicted_trait
  # pick out the columns that are directly or indirectly connected to the outcome column
  rho_pMax_gene = rho_pMax_predictors[selected_gene_index,selected_gene_index]
  rho_pMax_gene_outcome = 2*pnorm(-abs(Selected_genes$zMin.y)) ##
  ...
  Effects_pvalue_filepath = paste0(Effects_prefix,Effects_filename,".csv")
  fwrite(as.data.frame(Effects_mat),Effects_pvalue_filepath)
